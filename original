...
#AutoGrow 3.1.1 is free software: you can redistribute it and/or
...
def define_defaults(): # this function is first to make it easy for the user to modify the default parameters (see, especially, the boxes below)
    ...
    vars['mgltools_directory'] = "/home/xadmin/mgltools/"
    vars['openbabel_bin_directory'] = "/usr/local/lib/openbabel/2.4.1"
    vars['vina_executable'] = "/home/xadmin/Documents/autodockvina/bin/vina"
    vars['autoclickchem_script'] = "/home/xadmin/Documents/autoclickchem.py"
    vars['nn1_script'] = "/home/xadmin/Documents/NNScore.py"
    vars['nn2_script'] = "/home/xadmin/Documents/NNScore2.01.py"
    #############################################################################################################################################################################
    ...
    ###############################################################################################################################################################
    vars['babel_executable'] = "/usr/bin/babel"        # vars['babel_executable'] = "/home/myname/openbabel-2.2.0/bin/babel"
    vars['obprop_executable'] = "/usr/bin/obprop"      # vars['obprop_executable'] = "/home/myname/openbabel-2.2.0/bin/obprop"
    vars['prepare_ligand4.py'] = "/home/xadmin/mgltools/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py"    # vars['prepare_ligand4.py'] = "/home/myname/MGLTools-1.5.4/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py"
    vars['prepare_receptor4.py'] = "/home/xadmin/mgltools/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py"  # vars['prepare_receptor4.py'] = "/home/myname/MGLTools-1.5.4/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py"
    vars['mgl_python'] = "/home/xadmin/mgltools/bin/pythonsh"      # vars['mgl_python'] = "/home/myname/MGLTools-1.5.4/bin/pythonsh"
    ###############################################################################################################################################################


    vars['number_of_mutants'] = 400
    vars['number_of_crossovers_first_generation'] = 50
    vars['number_of_mutants_first_generation'] = 400
    vars['number_of_crossovers'] = 60
    vars['top_ones_to_advance_to_next_generation'] = 20
    vars['num_generations'] = 100
    vars['max_seconds_per_generation'] = 100000000 # about a week and a half
    ...
    vars['num_processors'] = 1 # with this setting, the program will use all processors available

    vars['directory_of_source_compounds'] = "/home/xadmin/Documents/intermediates.tmp"
    vars['directory_of_fragments'] = "/home/xadmin/Documents/examples"
    vars['filename_of_receptor'] = "/home/xadmin/Documents/4yzm.pdb"
    date = time.asctime(time.localtime(time.time()))
    vars['output_dir'] = "." + os.sep + "Parallel Experiment 2018-03-16" + os.sep
    ...
    vars['minimum_core_atoms_required'] = 5
...
class multi_threading():
    ...
    def __init__(self, inputs, task_class_name, variables_to_pass):
        ...
        """
        log('multi-thread job is launched')
        ...
        if num_processors == 1: # so it's running on just one processor, don't even try to parallelize it
            ...
            for inp in inputs:
                run_class.value_func(inp, variables_to_pass)
...
class run_autoclick_multithread(general_task):
    ...
    def value_func(self, command, variables_to_pass):
        ...
        log("Calculating value with autoclick")
        ...
        # pick a fragment
        log("Picking fragment...")
        frag_dir = random.choice(variables_to_pass['frag_dirs']) # pick the kind of fragment
        while len(variables_to_pass['fragments'][frag_dir]) == 0:
            print "Re-picking...."
            frag_dir = random.choice(variables_to_pass['frag_dirs'])
        ...
        for lig in new_ligs_filenames:
            get_time()
            log("Making mutant: " + os.path.basename(ligand) + " + " + os.path.basename(fragment) + " => " + os.path.basename(lig))
...
class run_ligmerge_multithread(general_task):
    ...
    def value_func(self, command, variables_to_pass):
        ...
        """
        log("Calculating value in silico reaction")
        global vars
        ...
        while True:
            ...
            if ligand1 != ligand2:
                break # you've got different names
            if count2 > 10000:
...
class druglike_multithread(general_task):
    ...
    def value_func(self, filename, variables_to_pass):
        ...
        """
        log("Checking the value of druglike properties...")
        check_time_limit_reached()
...
class LigMerge:
    ...
    def __find_combinations(self, combination, new_list, return_value):
        ...
        for index in range(len(temp)):
            ...
            if len(new_list) > 0:
                self.__find_combinations(copy.deepcopy(combination), copy.deepcopy(new_list), return_value)
    ...
    def main(self, pdb1, pdb2, all_symmetry_relations, all_substituent_combinations, output_mcs, max_runtime):
        ...
        if substructure_indices == []:
            log("No substructure indices!")
            return None, "No candidate for MCS with at least 3 atoms has been identified!!"
        ...
        for substructure_combinations_index in range(lower_bound, upper_bound):
            ...
            if (all_substituent_combinations == "TRUE"):
                ...
                if not len(fragments_with_same_handles) == 0:
                    self.__find_combinations(start_combinations, fragments_with_same_handles, return_value_find_combinations)
        ...
        log(str(len(pdbs_list)) + " crossovers generated. =========================================")
        return pdbs_list, ""
...
class CommonSubstructures:
    ...
    def get_common_structures(self, pdb1, pdb2, start_time, time_limit):
        ...
        for t in range(50): # so the maximum size of common fragments is 50 heavy atoms. probably way to big.
            ...
            while len(similar_substructures) < 1:
                log("Finding substructures")
                    ...
                    for item2 in pdb2_lists:
                        if item1[1] == item2[1]: # so element list is the same (you are comparing lists, e.g. [C, S, C] against [C, S, C])
                            ...
                            if key not in no_repeats: # so this structure has not been detected before
                                print item1
                                addddd = (item1[0], item2[0])
                                similar_substructures.append(addddd)
        ...
        '''
        while len(most_similar_substructures) < 0:
            counter += 1
            if counter > 10000:
                log("I've tried 10,000 times to do dis but it doesn't wok.")
                sys.exit(0)
...
def run_autoclick_chem(args):
    ...
    for react1 in reactants1:
        log("Appending reaction1")
        reactions.append([react1])
    for react1 in reactants2:
        log("Appending reaction2")
        reactions.append([react1])
    # now get all combinations of reactant1 x reactant2
    for react1 in reactants1:
        for react2 in reactants2:
            log("Putting reactions together: " + str(react1) + " + " + str(react2))
            reactions.append([react1,react2])

    filenames_created = []

    for reaction in reactions: # go through each of the reactions
        log("Going through reactions")
        percent_complete = reactions.index(reaction) / len(reactions) * 100
        log(str(int(percent_complete)) + "% Complete")
        ...
        log("Trying to react " + file1 + " and " + file2)
        ...
        if len(pdb_list) > 0: # if there some products, need to save them
            ...
            while True: # this just to make sure you don't repeat the filename
                log("Making sure a filename doesn't repeat...")
                newindex = random.randint(0,1000000)
...
def check_drug_likeness(pdb_filename):
    ...
    if lipinski == None or lipinski_strict == None or ghose == None:
        counter = 0
        while not os.path.exists(pdb_basename+".info"):
            counter+=1
            print "counter: " + str(counter)
            print "pdb filename:" + str(pdb_filename)
            print "end file: " + str(pdb_basename) + ".info"
            subprocess.call(["obprop '"+pdb_filename+"' > '"+pdb_basename+".info' "+"2>&1"],shell=True)
            if counter>100000:
                log("Obprob not running... Hmmm... exiting")
                sys.exit(0)
        print counter
...
def make_mutants(directory):
    ...
    if len(glob.glob(directory + "*.mutant.pdb")) < vars['number_of_mutants_current_generation']: # so you need to make more mutants
        ...
        while len(glob.glob(directory + "*.mutant.pdb")) < vars['number_of_mutants_current_generation']: # keep generating new mutants until you've passed the allowable level
            log("Creating one ligand... Number " + str(len(glob.glob(directory + "*.mutant.pdb")))) # Log each time it repeats
            get_time()
            multi_threading(range(20), run_autoclick_multithread, vars_to_pass) # so, performing 100 ligands at a time
...
def convert_ligand_pdb_files_to_pdbqt(directory):
    ...
    for filename in glob.glob(directory + "*.pdb"):
        if not os.path.exists(filename + "qt"):
            need_to_make_pdbqt.append(filename)
    ...
    print jobs
    multi_threading(jobs, execute_command, {'seconds_per_job': 1000, 'log_filename':directory + "support" + os.sep + "1.make_pdbqt"}) # give it all the time it needs to make these receptor pdbqt files
...
def convert_pdb_to_pdbqt_acceptable_format(filename):
    ...
    # Make conversion
    subprocess.call([vars["babel_executable"]+" '"+filename+"' -O '"+filename+"qt'"],shell=True)
...
def convert_receptor_pdb_files_to_pdbqt():
    ...
    while not os.path.exists(vars['filename_of_receptor'] + "qt"):
                print (count)
                count = count + 1
                convert_pdb_to_pdbqt_acceptable_format(vars['filename_of_receptor'])

                if count > 100:
                        log("ERROR: I've tried 100 times to convert the file \"" + vars['filename_of_receptor'] + "\" to the PDBQT format. Aborting program...")
                        sys.exit(0)
                ...
                # create a file to run the pdbqt making (from command line or through qsub... should eventually be two options...)
                jobs = []
                for filename in need_to_covert_receptor_to_pdbqt: jobs.append(vars['mgl_python'] + " " + vars['prepare_receptor4.py'] + " -r " + filename + " -o " + filename)
...
def dock_compounds(directory):
    ...
    for filename in glob.glob(directory + "*.pdbqt"):
        print "================================"
        print "FILE: " + filename
        need_to_dock.append(filename)

    # do the docking of the needed ligands
    jobs = []
    for receptor_filename in receptors:
        for lig_filename in need_to_dock:
            receptor = "/home/xadmin/Documents/4yzm.pdbqt"
            ligand = lig_filename
            out = lig_filename + ".vina"
            
            command = "vina --receptor '"+receptor+"' --ligand '"+ligand+"' --center_x -20 --center_y -18 --center_z -30 --size_x 30 --size_y 30 --size_z 30 --cpu 4 --out '"+out+"'"

            args = shlex.split(command)
            print args
            try:
                foo = subprocess.check_output(args)
            except:
                continue
...
def delete_bad_ligands(current_generation_dir):
    ...
    """
    log("doing stuff... YAY! Activity: deleting bad ligands")
    global vars
    ...
    print "There are " + str(len(glob.glob(current_generation_dir + "*.crossover.pdb"))) + " crossovers in the current directory"
    while len(glob.glob(current_generation_dir + "*.crossover.pdb")) > vars['number_of_crossovers_current_generation']:
        ...
        delete_all_associated_files(filename)
    print "Now, there are " + str(len(glob.glob(current_generation_dir + "*.crossover.pdb"))) + " crossovers in the current directory"
    print "There are " + str(len(glob.glob(current_generation_dir + "*.mutant.pdb"))) + " crossovers in the current directory"
    while len(glob.glob(current_generation_dir + "*.mutant.pdb")) > vars['number_of_mutants_current_generation']:
        ...
        delete_all_associated_files(filename)
    print "Now there are: " + str(len(glob.glob(current_generation_dir + "*.mutant.pdb")))
    if str(len(glob.glob(current_generation_dir + "*.mutant.pdb"))) != vars['number_of_mutants_current_generation']:
        print "=================================================\nThere are "+str(len(glob.glob(current_generation_dir + "*.mutant.pdb")))+" mutants and the target number is " + str(vars['number_of_mutants_current_generation'])
    if str(len(glob.glob(current_generation_dir + "*.crossover.pdb"))) != vars['number_of_crossovers_current_generation']:
        print "=================================================\nThere are "+str(len(glob.glob(current_generation_dir + "*.crossover.pdb")))+" crossovers and the target number is " + str(vars['number_of_crossovers_current_generation'])
...
def remove_duplicates(current_directory):
    ...
    for filename in glob.glob(current_directory + '*.pdb'):
        ...
        if not os.path.exists(can_filename): subprocess.call([vars['babel_executable'] + ' -ipdb "' + filename + '" -ocan -O "' + can_filename+'"'],shell=True)
...
class run_main():
    ...
    def __init__(self, args):
        ...
        while generation_num < vars['num_generations']:
            ...
            total_num_compoounds = len(glob.glob(current_generation_dir + "*.pdb"))

            if len(glob.glob(current_generation_dir + "*.pdbqt")) == total_num_compoounds and len(glob.glob(current_generation_dir + "*.vina")) == total_num_compoounds:
                ...
                log("Next Gen! Yay! Number: " + str(generation_num))
                log("Time passed: " + str(time.time() - time_at_the_beginning))
                get_time()
